...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { success: false, error: "Server error fetching listings." },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const listingData = {
      ...
      bedsPerRoom: Number(formData.get("bedsPerRoom")),
      availableBeds: Math.min(
        Number(formData.get("availableBeds")),
        Number(formData.get("bedsPerRoom"))
      ),
