...
import { Send, Upload, Wifi, AirVent, UtensilsCrossed, ParkingCircle, Bed, Table, WashingMachine } from "lucide-react";
  ...
  return (
    <main className="bg-neutral-50 min-h-screen py-12 px-4">
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-neutral-200/80">
        <h1 className="text-3xl font-bold text-neutral-900 mb-2">Create a New Listing</h1>
        <p className="text-neutral-500 mb-8">Fill out the details below to list your property on Rentify.</p>
        <form onSubmit={handleSubmit} className="space-y-8">
          {/* Basic Info */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="title" className="block text-sm font-medium text-neutral-700">Title</label>
              <input type="text" name="title" id="title" required value={form.title} onChange={handleChange} placeholder="e.g., Spacious 2BHK near PICT" className="mt-1 block w-full border-neutral-300 rounded-lg shadow-sm py-2 px-3 focus:ring-2 focus:ring-teal-500 transition" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div><label htmlFor="deposit" className="block text-sm font-medium text-neutral-700">Deposit (₹)</label><input type="number" name="deposit" id="deposit" required value={form.deposit} onChange={handleChange} placeholder="e.g., 20000" className="mt-1 block w-full border-neutral-300 rounded-lg shadow-sm py-2 px-3 focus:ring-2 focus:ring-teal-500 transition" /></div>
            <div><label htmlFor="rentPerMonth" className="block text-sm font-medium text-neutral-700">Rent per Month (₹)</label><input type="number" name="rentPerMonth" id="rentPerMonth" required value={form.rentPerMonth} onChange={handleChange} placeholder="e.g., 8500" className="mt-1 block w-full border-neutral-300 rounded-lg shadow-sm py-2 px-3 focus:ring-2 focus:ring-teal-500 transition" /></div>
            <div><label htmlFor="maintenance" className="block text-sm font-medium text-neutral-700">Maintenance (₹)</label><input type="number" name="maintenance" id="maintenance" value={form.maintenance} onChange={handleChange} placeholder="e.g., 1500 (optional)" className="mt-1 block w-full border-neutral-300 rounded-lg shadow-sm py-2 px-3 focus:ring-2 focus:ring-teal-500 transition" /></div>
            <div><label htmlFor="electricityBillBy" className="block text-sm font-medium text-neutral-700">Electricity Bill</label><select name="electricityBillBy" id="electricityBillBy" value={form.electricityBillBy} onChange={handleChange} className="mt-1 block w-full border-neutral-300 rounded-lg shadow-sm py-2 px-3 focus:ring-2 focus:ring-teal-500 transition"><option value="Owner">Owner pays</option><option value="Tenant">Tenant pays</option><option value="Shared">Shared</option></select></div>
            <div><label htmlFor="furnished" className="block text-sm font-medium text-neutral-700">Furnishing</label><select name="furnished" id="furnished" value={form.furnished} onChange={handleChange} className="mt-1 block w-full border-neutral-300 rounded-lg shadow-sm py-2 px-3 focus:ring-2 focus:ring-teal-500 transition"><option value="Furnished">Furnished</option><option value="Semi-furnished">Semi-furnished</option><option value="Unfurnished">Unfurnished</option></select></div>
          </div>
          {/* Description */}
          <div>
            <label htmlFor="description" className="block text-sm font-medium text-neutral-700">Description</label>
            <textarea name="description" id="description" rows={4} required value={form.description} onChange={handleChange} placeholder="Describe the property, rules, and nearby landmarks..." className="mt-1 block w-full border-neutral-300 rounded-lg shadow-sm py-2 px-3 focus:ring-2 focus:ring-teal-500 transition"></textarea>
          </div>
          {/* Amenities */}
          <div>
            <label className="block text-sm font-medium text-neutral-700">Amenities</label>
          ...
          <div className="pt-4 text-right">
            <button type="submit" disabled={isSubmitting} className="inline-flex items-center justify-center gap-2 py-3 px-6 border border-transparent shadow-sm text-base font-semibold rounded-full text-white bg-teal-600 hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 transition-all disabled:bg-neutral-400 disabled:cursor-not-allowed">
              <Send size={18} /> {isSubmitting ? 'Publishing...' : 'Publish Listing'}
            </button>
          </div>
        </form>
      </div>
